...
(function ($) {
    ...
    var easyWizardMethods = {
            ...
            return this.each(function () {
                ...
                if (thisSettings.steps > 1) {
                    ...
                    $steps.each(function (index) {
                        ...
                        //Custom - Yahya Khraishi
                        if (thisSettings.culture == 'en') {
                            $(this).css({
                                'float': 'left',
                        ...
                        else if (thisSettings.culture == 'ar') {
                            $(this).css({
                                'float': 'right',
                                'width': thisSettings.width,
                                'display': 'none'
        ...
        goToStep: function (step) {
            ...
            $nextStep.css('display', 'block').addClass('active');
            $nextStep.find('input, textarea, select, button').removeAttr('tabindex');

            //Yahya
            //Animate via Opacity instead of sliding
            var currentStep = $activeStep.attr('data-step');
            var nextStep = $nextStep.attr('data-step');
            if (nextStep > currentStep) {
                $activeStep.stop(true, true).animate({
                    opacity: '0'
                }, 600, function () {
                    $activeStep.css({ display: 'none' });
                    $nextStep.css({ display: 'block', opacity: '1' });
                });
            }
            else
            {
                ...
                $nextStep.stop(true, true).animate({
                    opacity: '1'
                }, 600, function () {
                    $nextStep.css({ display: 'block', opacity : '1' });
                });
        ...
        //Custom method -- solution to resizing using easyWizard
        resize: function (activeStepObj) {
            ...
            activeStepObj.addClass('active').css('opacity', '1').css('display', 'block');
